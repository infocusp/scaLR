...
pip install -e .
```
CPU is the default `torch` install; for CUDA, install a CUDA-specific `torch` build per the [PyTorch instructions](https://pytorch.org/get-started/locally/) before or after.
...
### Simple API layer (`scalr/api.py`, `scalr/artifact.py`, `scalr/result.py`, `scalr/validation.py`, `scalr/genes.py`, `scalr/calibration.py`, `scalr/metrics.py`, `scalr/leakage.py`)

This is a newer, self-contained layer built on top of the same engine (`scalr.nn.model`, etc.) but bypassing the config/pipeline machinery for simple in-memory workflows:

- `scalr.train(adata, labels_key, group_key=...)` does leakage-safe splitting (`scalr/leakage.py`), class-weighted training, temperature-scaling calibration (`scalr/calibration.py`), and macro/weighted-F1 + balanced-accuracy evaluation (`scalr/metrics.py`), returning an `AnnotationModel`.
- `AnnotationModel.save()`/`scalr.load_model()` (`scalr/artifact.py`) persist/restore a **self-contained model artifact directory** (`manifest.json`, `model.pt`, `model_config.json`, `label_mapping.json`, `features.json`, `preprocessing.json`, `calibration.json`, `metrics.json`) — no external config needed to reload and run inference.
