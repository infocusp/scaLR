...
def nn_feature_chunking(train_data, val_data, target:str, model_config:dict, chunksize:int, k:int, aggregation_strategy:str, dirpath:str, device:str='cpu') -> list[str]:
    ...
    for start in range(0, len(train_data.var_names), chunksize):
        ...
        trainer_ = Trainer(model, opt, lr, l2, loss_fn, callbacks, device, filepath_)
    ...
    for file in best_model_weights:
        ...
        weights = torch.load(file)['model_state_dict']['out_layer.weight'].cpu()
        all_weights.append(weights)

    full_weights_matrix = torch.cat(all_weights, dim=1)
    feature_class_weights = pd.DataFrame(full_weights_matrix, columns=train_data.var_names, index=id2label)
