...
class DgePseudoBulk(AnalysisBase):        
    ...
    def _make_design_matrix(self,
        ...
        for condition in self.factor_categories:
            condition_subset = fix_data[fix_data.obs[self.design_factor] == condition]
            for sum_sample in condition_subset.obs[self.sum_column].unique():
                sum_subset = condition_subset[condition_subset.obs[self.sum_column] ==
                                              sum_sample]
                subdata = ad.AnnData(
    ...
    def get_differential_expression_results(self,
        ...
        result_stats = DeseqStats(deseq_dataset,
                                  contrast=(self.design_factor_no_undrscr, *self.factor_categories))
    ...
    def plot_volcano(self,
        ...
        unsignificant_gene = dge_results_df['-log10(pvalue)'] <= (neg_log10_pval)
        ...
        plt.scatter(dge_results_df.loc[signi_genes_between_up_n_down,
                                       'log2FoldChange'],
                    dge_results_df.loc[signi_genes_between_up_n_down,
                                       '-log10(pvalue)'],
                    color='green',
                    alpha=0.2,
                    s=20,
                    label=f'-{self.fold_change}<FC<{self.fold_change} & p_val<={self.p_val}')
    ...
    def generate_analysis(self,
        ...
        assert self.factor_categories[0] in test_data.obs[self.design_factor].unique(
        ), f"{self.factor_categories[0]} must belong to {self.design_factor}"
        assert self.factor_categories[1] in test_data.obs[self.design_factor].unique(
        ), f"{sef.factor_categories[1]} must belong to '{self.design_factor}' column"
    
        cell_type_list = [condition for condition in test_data.obs[self.celltype_column].unique()]
        ...
        for cell_type in cell_type_list:
            ...
            plot = self.plot_volcano(dge_results_df,
                                      cell_type,
                                      dirpath)
            plt.close(plot)
